...
public interface PurchaseRepository extends JpaRepository<Purchase, Long> {
    @Query("SELECT new ru.isands.test.estore.dto.PurchaseCountByEmployeeDTO(p.employee.id, COUNT(p.electroItem.id), p.employee.lastName, p.employee.firstName, p.employee.patronymic, p.employee.birthDate, p.employee.position.id, p.employee.shop.id, p.employee.gender) " +
            "FROM Purchase p " +
            "WHERE p.purchaseDate >= :leftDate " +
            "AND p.purchaseDate <= :rightDate " +
    ...
    List<PurchaseCountByEmployeeDTO> getTotalCountByEmployee(@Param("leftDate") LocalDateTime leftDate, @Param("rightDate") LocalDateTime rightDate);
    ...
    List<PurchaseAmountByEmployeeDTO> getTotalAmountByEmployee(@Param("leftDate") LocalDateTime leftDate, @Param("rightDate") LocalDateTime rightDate);
